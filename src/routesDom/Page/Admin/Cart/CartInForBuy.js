      ...
      <div className="group-info-buy-cart">
        ...
        <div className="button-more-info">
          ...
          {cart.status_order && (
            <Button
              ...
              type="primary"
              className={cart.success ? 'btn-wait-order' : 'btn-success-order'}
              onClick={() => {
                onSuccess(id_cart);
              }}
            >
              {cart.success ? <LoadingOutlined /> : <CheckCircleOutlined />} Trạng Thái:{' '}
              {!cart.success ? 'Đã Phê Duyệt' : 'Chờ Phê Duyệt'}
            </Button>
          )}
          <Button
            type="primary"
        ...
        <div className="ground-address-cart">
          ...
          <div className="group-time-modal">
            ...
            <p>{`${moment(cart.timeCart).fromNow()}, ${moment(cart.timeCart).format('LLLL')}`}</p>
          </div>
          <div className="ground-user-by-cart">
            <ul>
              <li>
                ...
                <p>
                  {' '}
                  <Image style={{ width: '50px', borderRadius: '50%' }} src={userByCart.avatar} />
                </p>
              </li>
              <li>
                {' '}
                <span>Tên:</span>
                <p class="name-user-by-cart">{userByCart.name}</p>
              </li>
              <li>
                {' '}
                <span>Email:</span>
                <p>{userByCart.email}</p>
              </li>
            </ul>
          </div>
        </div>
