      ...
      if (values) {
        ...
        if (id_product) {
          ...
          if (fileListImage.length < 4) {
            notification['error']({
              message: 'Thông Báo',
              description: 'Vui lòng tải lên 4 ảnh',
            });
          } else {
  ...
  return (
    <>
      {linkNewProduct && (
        <h4>
          {' '}
          Click xem sản phẩm vừa {id_product ? 'cập nhật' : 'thêm'}{' '}
          <Link to={linkNewProduct}>Tại Đây</Link>
        </h4>
      )}
      <h3>{id_product ? 'Chỉnh sửa' : 'Thêm mới'} sản phẩm </h3>
      <Form
        form={form}
        {...styleForm}
        onFinish={onFinish}
      ...
      >
        ...
        >
          <Radio.Group>
            ...
            <Radio value="nữ" defaultChecked>
              Nữ
            </Radio>
            <Radio value="nam, nữ" defaultChecked>
              Nam, Nữ
            </Radio>
          </Radio.Group>
        </Form.Item>
